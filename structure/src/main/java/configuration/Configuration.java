...
public class Configuration {
	private static final String FILE = "configuration.ini";
	...
	//#configuration of the read proccess
		...
		try{
			...
			while((line = reader.readLine()) != null){
				...
				if (field.getType().getTypeName().equals("boolean")){
					...
					field.set(this, Long.parseLong(line.split("=")[1]));
				} else {
	...
	public boolean validate(){
		...
		if (!result){
			...
			buffer.append("'specify 'print' option if you want to print proces into console\n");
			buffer.append("'specify 'read' option if you want to run read proces instead of create\n");
